...
//
// القراءة دون اتصال تحدث على الصفحات الحقيقية نفسها (/، /lyrics/[id]،
// /favorites، /playlists...) لا في واجهة منفصلة: كل صفحة تقرأ بياناتها من
// اللقطة المخزَّنة (عبر خطّافات useOfflineHome/useOfflineFavorites/… في
// src/hooks) حين يتعذّر التصيير على الخادم. لزيارة مسار سبق تخزينه أونلاين،
// يُعاد نفس HTML المخزَّن كما هو دون أي منطق إضافي. لمسار (أو استعلام بحث/صفحة)
// لم يُخزَّن بعينه، لا سبيل لتشغيل Prisma على الخادم دون اتصال إطلاقًا، فيخدم
// الـ SW غلاف "/offline" الاحتياطي (OfflineShell) — يبقى المسار الحقيقي في
// شريط العنوان، ويقرأ OfflineShell المسار الفعلي عبر window.location ليعرض
// نفس مكوّن الصفحة الحقيقية (لا نسخة منفصلة) بمعطيات فارغة، فتُكمِل القراءة من
// اللقطة تلقائيًا. لكي يعمل هذا الغلاف يجب أن تكون أصوله (JS/CSS/الخط) مخزَّنة
// مسبقًا — لذا نخزّنها عند التثبيت باستخراج روابط /_next/static من صفحة
// /offline نفسها (لا نعتمد على زيارة المستخدم لها وهو متصل — وهو أصلًا غير
// متوقَّع، فلا رابط إليها في التنقّل).

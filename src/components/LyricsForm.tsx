...
import { NewLyricScreen } from "./NewLyricScreen";
import { useLyricsForm, type LyricsFormValues } from "@/lib/use-lyrics-form";
...
export function LyricsForm({
  ...
  const form = useLyricsForm({ mode, lyricsId, initialValues });
  const { values, setValues, error, loading, duplicates, suggestions, showSuggestions, setShowSuggestions, checking, submit } =
    form;
  ...
  return (
    <>
      {/* الموبايل: شاشة غامرة بحجم الشاشة الكاملة — إنشاء فقط (لا تعديل بعد). */}
      {mode === "create" && (
        <div className="sm:hidden">
          <NewLyricScreen form={form} />
        </div>
      )}

      <form
        onSubmit={handleSubmit}
        className={`${mode === "create" ? "hidden sm:flex" : "flex"} flex-col gap-4 rounded-xl border border-neutral-200 bg-white p-6 shadow-sm`}
      >
        {error && (
